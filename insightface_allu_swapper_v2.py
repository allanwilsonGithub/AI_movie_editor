...
SHOW_STAGES = False
FACE_PIC = 'PICS/source_images/face/mel-2.png'
FACE_PIC_DIR = "PICS/source_images/face/"
CANVAS_PIC = 'PICS/source_images/canvas/1.jpeg'
CANVAS_DIR = 'PICS/source_images/canvas/'
OUTPUT_DIR = 'PICS/output_images/'
movie_source_file = 'jeans_box_sit_source.avi'
...
def do_the_swap(ALL_FACES, CANVAS_PIC, OUTPUT_DIR, filename,set_number):
    ## Get Target Faces
...
def generate_many_source_to_many_canvas():
    ...
    ALL_FACES = get_all_source_faces(FACE_PIC_DIR)
    for filename in os.listdir(CANVAS_DIR):
        CANVAS_PIC = f"{CANVAS_DIR}/{filename}"
        print(f"Face: {FACE_PIC}")
        print(f"Canvas: {filename}")
        print("-----------------------------------------")
        do_the_swap(ALL_FACES, CANVAS_PIC, OUTPUT_DIR, filename)
    ...
    for frame_pic in os.listdir(FRAME_SOURCE_DIR):
        ...
        for set_number in range(1,number_of_sets):
            if not os.path.exists(f"{FRAME_OUTPUT_DIR}{set_number}/{frame_pic}"):
                ...
                do_the_swap(ALL_FACES, CANVAS_PIC, FRAME_OUTPUT_DIR, frame_pic, set_number)
            else:
...
# TODO
# If input frame exists in output dir.
# If output frame exists in output dir. This is done but we should do it much earlier to be faster
# If no face just keep original frame
# If mans face use man source[0,1,2] etc same with women